...
bash bin/graph.sh query "MATCH (s:Session)-[:BY]->(p:Person) WHERE date(s.date) >= date() - duration('P7D') RETURN s.topic AS topic, p.name AS by, s.date AS date ORDER BY s.date DESC LIMIT 5"
